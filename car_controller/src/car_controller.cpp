...
*/
   ...
   /*rot_vel to steering_angl end*/ 
    ...
    target_Vth =z_mid_angle;                //210729 char change
...
void pid_control()
{
    ...
    set_vel(target_Vx,0,Vth2Steering(target_Vx,real_Vth)); //2100729 char change
    ROS_INFO("target_x %f target_th %f",target_x,target_th);
    ROS_INFO("erreal_x %f real_th %f",real_x,real_th);
...
int main(int argc, char **argv){
  ...
  last_time = ros::Time::now();
   ...
   ros::Rate loop_rate(100);//设置循环间隔，即代码执行频率 100HZ
    ...
    set_vel(0,0,0.05);  //2100727 char add
